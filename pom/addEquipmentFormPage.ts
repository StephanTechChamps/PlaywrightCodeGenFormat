...
export class AddEquipmentFormPage {
    ...
    async createAGV(name: string, maxWeight: number, softwareVersion: string, hostName: string, portNumber: number, twentyFeetContainerOffset: number) {
        ...
        await this.fillField(this.name, name);
        await this.fillField(this.maxWeight, maxWeight);
        await this.fillField(this.softwareVersion, softwareVersion);
        await this.fillField(this.hostName, hostName);
        await this.fillField(this.portNumber, portNumber);
        await this.fillField(this.twentyFeetContainerOffset, twentyFeetContainerOffset);
    ...
    async createARTG(name: string, maxWeight: number, craneId: number, maxTierHeight: number, softwareVersion: string,
        ...
        await this.homePage.clickCreateVehicleButton();
        await this.fillField(this.name, name);
        await this.fillField(this.craneId, craneId);
        await this.fillField(this.maxWeight, maxWeight);
        await this.fillField(this.maxTierHeight, maxTierHeight);
        await this.fillField(this.softwareVersion, softwareVersion);
        await this.fillField(this.hostName, hostName);
        await this.fillField(this.portNumber, portNumber);
        await this.fillField(this.stackProfilingPort, portNumber);
        await this.saveButton.click();
    }
    ...
    // async getFormFieldsForSpecificVehicleType(vehicleType: VehiclesTab, data: Record<string, string>) {
    //     const requiredFields = vehicleFieldMap(vehicleType)
    //     console.log(requiredFields);
    //
    //     for (const field of requiredFields) {
    //         const locator = this.fields[field];
    //         if (!locator) throw new Error(`Field "${field}" not found`);
    //
    //
    ...
    //                 await locator.click();
    //             } else {
    //                 await locator.fill(data[field]);
    //             }
    //
    // }}
    // async fillFieldsForVehicle(vehicleType: string, data: Record<string, string>) {
    //     const requiredFields = vehicleFieldMap[vehicleType];
    //     for (const field of requiredFields) {
    //         const locator = this.fields[field];
    //         if (!locator) throw new Error(`Field "${field}" not found`);
